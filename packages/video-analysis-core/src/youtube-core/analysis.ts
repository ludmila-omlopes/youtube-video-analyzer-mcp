...
const DEFAULT_LONG_VIDEO_MAX_RETRIES = 1;
...
function getGenerationTimeoutMs(params: Pick<LongToolInput, "timeoutSeconds">): number {
  return params.timeoutSeconds ? params.timeoutSeconds * 1000 : GENERATION_TIMEOUT_MS;
}

function getMaxRetries(params: Pick<LongToolInput, "maxRetries">): number {
  return params.maxRetries ?? DEFAULT_LONG_VIDEO_MAX_RETRIES;
}

function getMaxChunkDurationSeconds(params: Pick<LongToolInput, "maxChunkDurationSeconds">): number {
  return params.maxChunkDurationSeconds ?? CONSERVATIVE_URL_CHUNK_DURATION_SECONDS;
}

function splitChunkPlanByMaxDuration(
  chunkPlan: ChunkPlanItem[],
  maxChunkDurationSeconds: number,
  overlapSeconds: number
): ChunkPlanItem[] {
  const maxDuration = Math.max(MIN_CHUNK_DURATION_SECONDS, Math.floor(maxChunkDurationSeconds));
  const overlap = Math.max(0, Math.floor(overlapSeconds));
  const splitPlan: ChunkPlanItem[] = [];

  for (const chunk of chunkPlan) {
    let startOffsetSeconds = chunk.startOffsetSeconds;

    while (startOffsetSeconds < chunk.endOffsetSeconds) {
      const endOffsetSeconds = Math.min(startOffsetSeconds + maxDuration, chunk.endOffsetSeconds);
      splitPlan.push({
        index: splitPlan.length,
        startOffsetSeconds,
        endOffsetSeconds,
      });

      if (endOffsetSeconds >= chunk.endOffsetSeconds) {
        break;
      }

      startOffsetSeconds = Math.max(endOffsetSeconds - overlap, startOffsetSeconds + 1);
    }
  }
...
function categorizeUploadedFileFallback(error: DiagnosticError): string {
  if (error.code === "LOCAL_DEPENDENCY_MISSING") {
    return `${error.details?.missingDependency ?? "local dependency"} missing`;
  }
...
function createDownloadDiagnostic(
  ...
  const causeMessage = diagnostic.causeMessage?.toLowerCase() ?? "";
  const missingYtDlp =
    causeMessage.includes("yt_dlp") ||
    causeMessage.includes("yt-dlp") ||
    causeMessage.includes("no module named") ||
    causeMessage.includes("enoent") ||
    causeMessage.includes("not recognized");
  const missingFfmpeg = causeMessage.includes("ffmpeg");

  if (!missingYtDlp && !missingFfmpeg) {
    return diagnostic;
  }

  const missingDependency = missingFfmpeg ? "ffmpeg" : "yt-dlp";
  return new DiagnosticError({
    ...
    message:
      missingDependency === "ffmpeg"
        ? "ffmpeg is required to finalize downloaded video streams for uploaded-file analysis."
        : "yt-dlp is required for uploaded-file analysis but was not found.",
    retryable: false,
    strategyRequested: params.strategyRequested,
    strategyAttempted: params.strategyAttempted,
    cause: error,
    details: {
      ...
      missingDependency,
      recommendedStrategy: "url_chunks",
      installHintWindows: missingDependency === "ffmpeg" ? "winget install Gyan.FFmpeg" : "winget install yt-dlp.yt-dlp",
      installHintPython: missingDependency === "yt-dlp" ? "python -m pip install -U yt-dlp" : null,
      nextStep: "Install the missing dependency and restart the MCP client, or retry analyze_long_youtube_video with strategy=url_chunks.",
    },
  });
}
...
async function analyzeChunkedVideo(
  ...
  for (const chunk of params.chunkPlan) {
    ...
    for (let attempt = 0; attempt <= params.maxRetries; attempt += 1) {
      try {
        chunkResult = await generateStructuredJson(
          ai,
          {
            model: params.chunkModel,
        ...
        const diagnostic = asDiagnosticError(error, {
          ...
          code: "LONG_VIDEO_CHUNK_FAILED",
          stage: "chunk_generate",
          message: "Failed to analyze a long-video chunk.",
          strategyRequested: params.strategyRequested,
          strategyAttempted: params.strategyAttempted,
        });

        if (diagnostic.code === "REQUEST_CANCELLED" || !diagnostic.retryable || attempt >= params.maxRetries) {
          throw diagnostic;
        }

        context.logger.warn("long_video.chunk_retry", {
          chunkIndex: chunk.index,
          attempt: attempt + 1,
    ...
    if (!chunkResult) {
      throw new Error(`Chunk ${chunk.index} failed without returning a diagnostic error.`);
    }
  ...
  for (const chunk of params.chunkPlan) {
    ...
    try {
      try {
        ...
        throw createDownloadDiagnostic(error, {
          tool: context.tool,
      ...
      for (let attempt = 0; attempt <= params.maxRetries; attempt += 1) {
        try {
          ...
          context.logger.warn("long_video.uploaded_chunk_retry", {
            chunkIndex: chunk.index,
            attempt: attempt + 1,
            nextAttempt: attempt + 2,
            maxAttempts: params.maxRetries + 1,
...
async function runUrlChunkStrategy(
  ...
  const chunkPlan = splitChunkPlanByMaxDuration(
    plan.chunkPlan,
    getMaxChunkDurationSeconds(params),
    DEFAULT_CHUNK_OVERLAP_SECONDS
  );

  if (chunkPlan.length !== plan.chunkPlan.length) {
    context.logger.info("long_video.chunk_plan_capped", {
      originalChunkCount: plan.chunkPlan.length,
      cappedChunkCount: chunkPlan.length,
      maxChunkDurationSeconds: getMaxChunkDurationSeconds(params),
    });
  }
  ...
  const chunkedResult = await analyzeChunkedVideo(
    ai,
    {
      ...
      strategyRequested: config.strategyRequested,
      strategyAttempted: "url_chunks",
      generationTimeoutMs: getGenerationTimeoutMs(params),
...
async function runUploadedFileStrategy(
  ...
  try {
      ...
      try {
        const analysis = await generateStructuredJson(
          ...
          },
          {
            ...
            timeoutMs: getGenerationTimeoutMs(params),
            abortSignal: context.abortSignal,
          }
        );
    ...
    const cappedChunkPlan = splitChunkPlanByMaxDuration(
      chunkPlanResult.chunkPlan,
      getMaxChunkDurationSeconds(params),
      DEFAULT_CHUNK_OVERLAP_SECONDS
    );

    if (cappedChunkPlan.length !== chunkPlanResult.chunkPlan.length) {
      context.logger.info("long_video.uploaded_chunk_plan_capped", {
        originalChunkCount: chunkPlanResult.chunkPlan.length,
        cappedChunkCount: cappedChunkPlan.length,
        maxChunkDurationSeconds: getMaxChunkDurationSeconds(params),
      });
    }
